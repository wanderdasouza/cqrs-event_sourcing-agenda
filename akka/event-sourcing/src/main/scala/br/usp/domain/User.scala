...
object UserDomain {
  ...
  final case class CreateUser(user: User, replyTo: ActorRef[String]) extends Command
  final case class GetUser(replyTo: ActorRef[GetUserResponse]) extends Command
  final case class DeleteUser(replyTo: ActorRef[String]) extends Command
  ...
  case object UserDeleted extends Event
